...
This would mean that the user cannot route to the homepage without logging in to the application. To fix this bug, the route path will have to be rendered as a '/protected' path, so that any unauthenticated action gets redirected to the login page.

**Get the suburb areas of other areas in the greater Wellington region**<br>
This is a feature that needs to be added to be a more useful support tool for decision-makers. To add this feature, GeoJSON files will have to be found for other suburbs out of the direct Wellington City council area of governance. The GeoJSON file currently used was procured from the Wellington City council website, and other similar GeoJSON files can be found in other council websites. This should be fairly easy to include into the application as the backend support for this is already there.
...
This is a feature that needs to be added to be a more useful support tool for decision-makers. To add this feature, the database will have to be adapted to allow for the car and train data. The visualisation will have to also be changed to show the difference in the transport types.

**Add a clock to show what point in time is the animation iterating through**<br>
This is a feature that needs to be added to improve the user experience and allow for more concise information to be provided to the user. To add this feature, a clock will have to be added to the frontend, to show what point in time the animation is displaying. This will be done with the use of material UI components.
...
This is a feature that needs to be added to be a more useful support tool for decision-makers. To add this feature, a number will have to be displayed on the suburb legend and should be dynamic to match the changes in the suburb congestion colours.

**Time fields need to be reactive to the different dates.**<br>
Time of date stamps is not consistent across the dates. Therefore, some times do not have any information and do not display a visualisation. To fix this bug, times that do not have information on specific days will have to be filtered out.

**Group the zones, based on colour**<br>
This is a feature that needs to be added to be a more useful support tool for decision-makers. To add this feature, there will need to be a whole other action implemented which allows for this grouping to occur.
...
The delivered project meets the requirements of the amended scope. Several features were not developed in prior documentation. The team has set up a database to store the data resources which were much larger than originally designed.

In earlier reports, the team regarded the regional data to be a resource that would be readily available to the project and easy to procure, however, this was not the case. The data often represented a niche subject which took time for team members to understand. Project deployment was also an attribute not fully discussed in earlier reports and this was a big part of our project.
...
In a more technical aspect, the team will focus on having deployment done earlier in the future to have more client feedback to maintain the delivery of a stronger final product.
...
| Suburb Areas     | These points were used to disaggregate the areas on the map to display demographic information relevant to the travel population. | Wellington City Council |
| Spring Libraries | This is an open-source library used for the connection with the frontend in the backend.                                          | mvn repository          |
| Travel Data      | This is the data supplied to us by the client to visualise the information on the map.                                            | Google Maps API         |
| Maven            | This was used for testing and managing dependencies necessary to run the project                                                  | Apache Maven            |
