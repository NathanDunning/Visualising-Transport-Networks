...
import range from 'leaflet-range';
...
import {myTooltip} from "../components/navbar/navbarhelpers";
...
export const generateContours = (data) => {
    ...
    let currentState = store.getState().data;
    copy = data;
    ...
    let op = centres[store.getState().data.city];
    ...
    const grid = turf.interpolate(data, .05, {
        gridType: 'point',
        property: 'drivetime',
        units: 'degrees',
        weight: 1,
    ...
    let isochrone = getIsochroneType(isobands, path);

    isochrone.append('title')
        .text((d) =>
            parseIsoband(d.properties.drivetime).map(x => x / 60).join(' - ') + ' minutes'
        );
...
function myInterpolate(data) {
    ...
    let tin = turf.tin(data, 'drivetime');
    ...
    let results = [];
    turf.featureEach(f, function (gridFeature) {
        let zw = 0;
        let sw = 40;
        console.log(gridFeature);
        // calculate distance from each input point to the grid points
        turf.featureEach(data, function(point) {
            let gridPoint = gridFeature;
            console.log(gridFeature)
            let d = distance(op, gridPoint, options);
            let zVal = point.properties['drivetime'];
            console.log(d, zVal);

                let d2 = distance(op, point, options);
        ...
        zw = zw;
        // write to points
        let newFeature = clone(gridFeature);
        console.log(zw);
...
function addCollectionPoints(collection, map) {
    ...
    let points = L.geoJson(collection, pointStyle);
...
function createLegends(map, isobands, max, interval) {
    if(!(legend1===null))
        ...
        legend1.onAdd = () => {
            let parent = L.DomUtil.create('div', 'legend-container');
            d3.select(parent).append('p');
            let div = L.DomUtil.create('div', 'legend', parent),
                len = isobands.features.length;
            let divs = d3.select(div)
                .selectAll('div')
...
export const drawErrorBox = (text) => {
    if (text===undefined && !(errorbox===undefined)) {
        ...
        if (errorbox === undefined) {
            let elem = L.control({position: 'bottomleft'});
            elem.onAdd = () => {
...
export const addStepsSlider = () => {
    let customSlider = L.control.range({
    ...
    customSlider.on('input change', function(e) {
        console.log(e.value);
        if(+e.value===180) {
           STEPS = 240;
        }
